...
1. `src/consts.ts` 의 `AUTHORS` — `voice` 까지 채웁니다
...
- **BeepTimer 글 3편만 `임데브` 로 옮겼습니다.** 만든 앱의 devlog 라 학습 기록과 성격이 다릅니다. 옮기면서 `project: beeptimer` 도 같이 붙였습니다.
- ⚠ **이름과 `id` 가 다릅니다.** 화면은 `study Lim`·`임데브` 인데 주소는 `/authors/student`·`/authors/developer` 입니다. 이름은 바꿀 수 있지만 `id` 를 바꾸면 글 124편의 frontmatter 와 주소가 같이 움직입니다 — 이름만 고치세요.
