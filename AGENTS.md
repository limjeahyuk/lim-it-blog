...
전체 글 124편 중 113편이 학습 기록(임 Student)입니다.
...
| id | 이름 | 무엇을 쓰나 | 색 |
|---|---|---|---|
| `student` | 임 Student | 배운 것 — 수업 필기, 공부 정리 | 파랑 |
| `developer` | 임 Developer | 만든 것 — devlog, 이 블로그를 고친 기록 | 빨강 |
...
```
[ Editors ]   [ 지면 ]   [ Live Service / Contact ]
...
- 목록 첫 꼭지는 `lead` 로 제목을 키웁니다 (`PostCard.astro`)
...
- **iOS 를 공부에 합쳤습니다.** 학습 기록 113편이 전부 `임 Student` 입니다. iOS·React·Flutter 를 저자로 쪼개면 "주제별 태그"를 이름만 바꾼 것이 됩니다 — 저자는 주제가 아니라 성격으로 나눕니다.
- **BeepTimer 글 3편만 `임 Developer` 로 옮겼습니다.** 만든 앱의 devlog 라 학습 기록과 성격이 다릅니다. 옮기면서 `project: beeptimer` 도 같이 붙였습니다.
- **저자 이름은 `임 Student` 만 정해진 것입니다.** `임 Developer` 는 제가 붙였습니다 — `consts.ts` 의 `name` 한 줄이라 언제든 바꿀 수 있습니다.
