...
**글 상세(`PostLayout.astro`)는 2열입니다.** 목록과 달리 왼쪽 카테고리 열이 없습니다.

```
[ 글 본문 ]   [ Profile / Works / Contact ]
  1fr                 17.5rem
```

- 컨테이너 `max-width: 82rem`, 본문 카드 976px · 글 폭 912px (1440px 화면 기준)
- **왼쪽 열을 넣지 않은 건 글 폭 때문입니다.** 카테고리 열까지 붙이면 본문이 좁아집니다. 글을 읽는 화면에서는 탐색보다 글 폭이 우선입니다.
- ⚠ **사이드바는 1200px 부터만 옆에 붙습니다.** 사이드바와 간격을 빼고도 본문이 예전 폭(864px)보다 좁아지지 않는 최소 지점이라 그렇습니다. 이 값을 내리면 노트북에서 글이 오히려 답답해집니다.
- 1200px 미만에서는 본문이 `max-width: 56rem` 한 열로 돌아가고 **사이드바는 글 아래**로 내려옵니다 (§6-4 와 같은 이유).
...
### 본문 편집기 — Decap 것을 안 씁니다

**`public/admin/editor.js` 가 Decap 의 markdown 위젯을 통째로 갈아끼웁니다.**
마크다운 원문 textarea + 직접 만든 툴바(제목·굵게·색·형광펜·사진·코드블록…)입니다.

왜 갈아끼웠냐면 **양쪽 다 못 쓰기 때문입니다.**

- 리치 텍스트(`rich_text`)는 폰에서 한글 입력이 끊기고 코드펜스를 망칩니다.
- 그래서 원문(`raw`)만 남겼더니, **원문 모드에서는 Decap 툴바 버튼이 전부 `disabled` 로 렌더됩니다.** 굵게도 사진도 안 됩니다.

저장되는 건 여전히 **마크다운 원문**입니다. 툴바는 커서 자리에 마크다운을
꽂아 넣을 뿐이라, 손으로 쓴 글 117개와 형식이 같습니다.

- **글자 색은 `<span class="c-teal">` 처럼 클래스로 넣습니다.** 색 값을 본문에 박으면 다크·라이트 한쪽에서 반드시 안 읽힙니다. 실제 색은 `global.css` 가 테마별로 고릅니다 — **색을 늘리려면 `editor.js` 의 `COLORS` 와 `global.css` 의 `.prose .c-*` 를 같이** 고쳐야 합니다.
...
- ⚠ **`document.execCommand('insertText')` 결과를 믿으면 안 됩니다.** 크롬·사파리에는 "똑똑한 교체"가 있어서, 단어를 통째로 고른 채 무언가를 넣으면 **옆 띄어쓰기를 저 혼자 지웁니다.** 그래서 넣은 뒤 결과를 대조하고 어긋나면 직접 갈아끼웁니다. execCommand 를 쓰는 이유는 그게 되돌리기(⌘Z)를 살려 주기 때문입니다.
...
### 2026-08-13 · 글 상세 화면 양옆이 비어 있던 것

글 목록에는 사이드바가 셋이나 붙는데 글을 열면 가운데 56rem 카드 하나만 덩그러니
남았습니다. 넓은 화면에서 양옆이 400px 넘게 비었습니다. 오른쪽에 Profile·Works·Contact
를 붙이고 글 폭도 912px 로 넓혔습니다.

- **왼쪽(카테고리·태그) 열은 일부러 안 붙였습니다.** 셋 다 붙이면 글이 좁아집니다 — 넓혀 달라는 요청과 반대로 갑니다. 글을 읽는 화면이라 탐색보다 글 폭을 골랐습니다.
- 사이드바가 붙는 기준을 1024px 이 아니라 **1200px** 로 뒀습니다. 그보다 좁으면 사이드바 폭을 빼고 나서 글이 예전보다 좁아집니다. 넓히려다 좁아지는 구간을 안 만들려는 것입니다.
- 사이드 컴포넌트는 피드가 쓰던 것을 그대로 씁니다. 프로필 내용이 두 곳에 갈라지지 않게 `SideProfile`·`SideWorks`·`SideContact` 를 재사용했습니다.
