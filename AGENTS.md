...
2. 본문에 나오는 첫 사진
3. `public/images/default-cover.svg`

- ⚠ **기본 커버에 글자를 넣지 마세요.** 커버 위에 제목을 얹는 지면에서는 그림 속 글자와 제목이 겹칩니다 (한 번 넣었다가 뺐습니다).
...
- ⚠ **기본 커버가 라이트 테마에서 튑니다.** 어두운 판이라 흰 배경에서 검은 사각형으로 도드라집니다. 홈을 정하고 나면 테마별로 다른 그림을 쓰든지 밝은 판을 하나 더 만들어야 합니다.
